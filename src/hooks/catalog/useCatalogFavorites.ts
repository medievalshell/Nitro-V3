...
import {
    ...
    getPagesStorageKey,
    IFavoriteOffer,
    LEGACY_STORAGE_KEY_OFFERS,
    LEGACY_STORAGE_KEY_PAGES,
...
const useCatalogFavoritesState = () => {
    ...
    const toggleFavoriteOffer = useCallback(
        (offerId: number, name?: string, iconUrl?: string) => {
    ...
    const isFavoritePage = useCallback(
        (pageId: number) => {
    ...
    const getFavoriteOffer = useCallback(
        (offerId: number): IFavoriteOffer | undefined => {
