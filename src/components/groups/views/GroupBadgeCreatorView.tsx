                                ...
                                <Grid columnCount={3} gap={1}>
                                    {POSITIONS.map((position, posIndex) => {
                                        return (
                                            <div
                                                key={posIndex}
                                                className={`relative rounded-[.25rem] w-[16px] h-[16px] bg-[#fff] border-2 border-[solid] border-[#fff] [box-shadow:inset_3px_3px_#0000001a] cursor-pointer ${badgeParts[index].position === position ? 'bg-primary [box-shadow:none]' : ''}`}
                    ...
                    {(badgeParts[selectedIndex].type === GroupBadgePart.BASE ? groupCustomize.badgeBases : getAvailableSymbols()).map((item, index) => {
                        return (
                            <Column
                                ...
                                onClick={(event) => setPartProperty(selectedIndex, 'key', item.id)}
                            >
