...
export default defineConfig({
    ...
    server: {
        ...
        proxy: {
            ...
            // Local dev: proxy remote staging assets/gamedata/texts through the
            // dev server so the browser sees them same-origin (the remote host
            // sends no CORS headers). Lets the client point at relative
            // /nitro-staging-assets/* URLs.
            '/nitro-staging-assets': {
                target: 'https://hippiehotel.nl',
                changeOrigin: true,
                secure: true,
            }
        }
